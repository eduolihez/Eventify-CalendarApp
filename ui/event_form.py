...
class EventForm:
    ...
    def __init__(self, parent, db_manager, app, event_data=None, callback=None):
        ...
        self.window.title(self._("Add Event") if not event_data or "id" not in event_data else self._("Edit Event"))
        self.window.geometry("650x650")
        self.window.transient(parent)
        self.window.grab_set()
        
        # Apply theme
        self.window.configure(bg=self.app.theme_manager.themes[self.app.theme_manager.current_theme]["bg"])
    ...
    def create_form(self):
        ...
        header_text = self._("Add Event") if not self.event_data or "id" not in self.event_data else self._("Edit Event")
        ttk.Label(
            header_frame, 
            text=header_text,
            style="Title.TLabel"
        ...
        # Title
        ttk.Label(main_frame, text=self._("Title:"), style="TLabel", font=("SF Pro Display", 13, "bold")).grid(row=1, column=0, sticky="w", pady=(0, 15))
        self.title_var = tk.StringVar(value=self.event_data.get("title", ""))
        title_entry = ttk.Entry(main_frame, textvariable=self.title_var, font=("SF Pro Display", 13))
        title_entry.grid(row=1, column=1, sticky="ew", pady=(0, 15))
        
        # Description
        ttk.Label(main_frame, text=self._("Description:"), style="TLabel", font=("SF Pro Display", 13, "bold")).grid(row=2, column=0, sticky="nw", pady=(0, 15))
        ...
        self.description_text = tk.Text(
            ...
            fg=self.app.theme_manager.themes[self.app.theme_manager.current_theme]["fg"],
            bd=0,
            padx=8,
            pady=8
        )
        ...
        # Start time
        ttk.Label(main_frame, text=self._("Start Time:"), style="TLabel", font=("SF Pro Display", 13, "bold")).grid(row=3, column=0, sticky="w", pady=(0, 15))
        ...
        ttk.Label(start_frame, text=self._("Date:"), style="TLabel").pack(side=tk.LEFT, padx=(0, 5))
        ...
        ttk.Label(start_frame, text=self._("Time:"), style="TLabel").pack(side=tk.LEFT, padx=(0, 5))
        ...
        ttk.Label(start_frame, text=":", style="TLabel", font=("SF Pro Display", 12, "bold")).pack(side=tk.LEFT)
        ...
        # End time
        ttk.Label(main_frame, text=self._("End Time:"), style="TLabel", font=("SF Pro Display", 13, "bold")).grid(row=4, column=0, sticky="w", pady=(0, 15))
        ...
        ttk.Label(end_frame, text=self._("Date:"), style="TLabel").pack(side=tk.LEFT, padx=(0, 5))
        ...
        ttk.Label(end_frame, text=self._("Time:"), style="TLabel").pack(side=tk.LEFT, padx=(0, 5))
        ...
        ttk.Label(end_frame, text=":", style="TLabel", font=("SF Pro Display", 12, "bold")).pack(side=tk.LEFT)
        ...
        # Location
        ttk.Label(main_frame, text=self._("Location:"), style="TLabel", font=("SF Pro Display", 13, "bold")).grid(row=5, column=0, sticky="w", pady=(0, 15))
        ...
        # Priority
        ttk.Label(main_frame, text=self._("Priority:"), style="TLabel", font=("SF Pro Display", 13, "bold")).grid(row=6, column=0, sticky="w", pady=(0, 15))
        priority_frame = ttk.Frame(main_frame, style="TFrame")
        priority_frame.grid(row=6, column=1, sticky="ew", pady=(0, 15))
        
        self.priority_var = tk.StringVar(value=self.event_data.get("priority", "medium"))
        
        # Create priority buttons with iOS-style appearance
        low_color = self.app.theme_manager.themes[self.app.theme_manager.current_theme]["success"]
        medium_color = self.app.theme_manager.themes[self.app.theme_manager.current_theme]["warning"]
        high_color = self.app.theme_manager.themes[self.app.theme_manager.current_theme]["danger"]
        
        # Low priority button
        low_frame = tk.Frame(priority_frame, bg=self.app.theme_manager.themes[self.app.theme_manager.current_theme]["bg"])
        ...
        low_fill = low_color if self.priority_var.get() == "low" else self.app.theme_manager.themes[self.app.theme_manager.current_theme]["button"]
        low_text_color = "white" if self.priority_var.get() == "low" else self.app.theme_manager.themes[self.app.theme_manager.current_theme]["fg"]
        
        low_rect = create_rounded_rectangle(
            low_canvas, 0, 0, 90, 36, 
            radius=18, 
            fill=low_fill, 
            outline=""
        )
        ...
        def set_low_priority(event):
            ...
            medium_canvas.itemconfig(medium_rect, fill=self.app.theme_manager.themes[self.app.theme_manager.current_theme]["button"])
            medium_canvas.itemconfig(medium_text, fill=self.app.theme_manager.themes[self.app.theme_manager.current_theme]["fg"])
            high_canvas.itemconfig(high_rect, fill=self.app.theme_manager.themes[self.app.theme_manager.current_theme]["button"])
            high_canvas.itemconfig(high_text, fill=self.app.theme_manager.themes[self.app.theme_manager.current_theme]["fg"])
        
        low_canvas.bind("<Button-1>", set_low_priority)
        
        # Medium priority button
        medium_frame = tk.Frame(priority_frame, bg=self.app.theme_manager.themes[self.app.theme_manager.current_theme]["bg"])
        ...
        medium_fill = medium_color if self.priority_var.get() == "medium" else self.app.theme_manager.themes[self.app.theme_manager.current_theme]["button"]
        medium_text_color = "white" if self.priority_var.get() == "medium" else self.app.theme_manager.themes[self.app.theme_manager.current_theme]["fg"]
        
        medium_rect = create_rounded_rectangle(
            medium_canvas, 0, 0, 90, 36, 
            radius=18, 
            fill=medium_fill, 
            outline=""
        )
        ...
        def set_medium_priority(event):
            ...
            low_canvas.itemconfig(low_rect, fill=self.app.theme_manager.themes[self.app.theme_manager.current_theme]["button"])
            low_canvas.itemconfig(low_text, fill=self.app.theme_manager.themes[self.app.theme_manager.current_theme]["fg"])
        ...
        high_frame = tk.Frame(priority_frame, bg=self.app.theme_manager.themes[self.app.theme_manager.current_theme]["bg"])
        ...
        high_fill = high_color if self.priority_var.get() == "high" else self.app.theme_manager.themes[self.app.theme_manager.current_theme]["button"]
        high_text_color = "white" if self.priority_var.get() == "high" else self.app.theme_manager.themes[self.app.theme_manager.current_theme]["fg"]
        
        high_rect = create_rounded_rectangle(
            high_canvas, 0, 0, 90, 36, 
            radius=18, 
            fill=high_fill, 
            outline=""
        )
        ...
        # Color
        ttk.Label(main_frame, text=self._("Color:"), style="TLabel", font=("SF Pro Display", 13, "bold")).grid(row=7, column=0, sticky="w", pady=(0, 15))
        color_frame = ttk.Frame(main_frame, style="TFrame")
        color_frame.grid(row=7, column=1, sticky="ew", pady=(0, 15))
        
        self.color_var = tk.StringVar(value=self.event_data.get("color", "#3498db"))
        
        # Create a color preview with rounded corners
        color_preview_frame = tk.Frame(color_frame, bg=self.app.theme_manager.themes[self.app.theme_manager.current_theme]["bg"])
        ...
        self.color_preview_rect = create_rounded_rectangle(
            color_preview_canvas, 0, 0, 36, 36, 
            radius=18, 
        ...
        # Recurring event
        ttk.Label(main_frame, text=self._("Recurring:"), style="TLabel", font=("SF Pro Display", 13, "bold")).grid(row=8, column=0, sticky="w", pady=(0, 15))
        recurring_frame = ttk.Frame(main_frame, style="TFrame")
        recurring_frame.grid(row=8, column=1, sticky="ew", pady=(0, 15))
        
        self.is_recurring_var = tk.BooleanVar(value=bool(self.event_data.get("is_recurring", False)))
        
        # Create a custom switch for recurring events
        switch_frame = tk.Frame(recurring_frame, bg=self.app.theme_manager.themes[self.app.theme_manager.current_theme]["bg"])
        ...
        switch_bg_color = self.app.theme_manager.themes[self.app.theme_manager.current_theme]["success"] if self.is_recurring_var.get() else self.app.theme_manager.themes[self.app.theme_manager.current_theme]["button"]
        self.switch_bg = create_rounded_rectangle(
            switch_canvas, 0, 0, switch_width, switch_height, 
            radius=18, 
        ...
        def toggle_recurring(event):
            ...
            if self.is_recurring_var.get():
                switch_canvas.itemconfig(self.switch_bg, fill=self.app.theme_manager.themes[self.app.theme_manager.current_theme]["success"])
                switch_canvas.coords(self.switch_handle, switch_width - 30, 6, switch_width - 6, 30)
                self.recurring_options_frame.grid()
            else:
                switch_canvas.itemconfig(self.switch_bg, fill=self.app.theme_manager.themes[self.app.theme_manager.current_theme]["button"])
        ...
        self.recurring_options_frame.grid(row=9, column=0, columnspan=2, sticky="ew", pady=(0, 15))
        
        # Recurrence type
        ttk.Label(self.recurring_options_frame, text=self._("Repeat:"), style="TLabel", font=("SF Pro Display", 12, "bold")).grid(row=0, column=0, sticky="w", pady=5)
        
        self.recurrence_type_var = tk.StringVar(value=self.event_data.get("recurrence_type", "daily"))
        recurrence_types = [
            ("daily", self._("Daily")),
        ...
        for i, (value, text) in enumerate(recurrence_types):
            radio_frame = tk.Frame(recurrence_type_frame, bg=self.app.theme_manager.themes[self.app.theme_manager.current_theme]["bg"])
            ...
            radio_fill = self.app.theme_manager.themes[self.app.theme_manager.current_theme]["secondary_accent"] if is_selected else self.app.theme_manager.themes[self.app.theme_manager.current_theme]["button"]
            radio_text_color = "white" if is_selected else self.app.theme_manager.themes[self.app.theme_manager.current_theme]["fg"]
            
            radio_rect = create_rounded_rectangle(
                radio_canvas, 0, 0, 90, 36, 
                radius=18, 
                fill=radio_fill, 
                outline=""
            )
            ...
            def set_recurrence_type(event):
                ...
                for child in recurrence_type_frame.winfo_children():
                    ...
                    fill_color = self.app.theme_manager.themes[self.app.theme_manager.current_theme]["secondary_accent"] if is_selected else self.app.theme_manager.themes[self.app.theme_manager.current_theme]["button"]
                    text_color = "white" if is_selected else self.app.theme_manager.themes[self.app.theme_manager.current_theme]["fg"]
        ...
        # End recurrence
        ttk.Label(self.recurring_options_frame, text=self._("End Recurrence:"), style="TLabel", font=("SF Pro Display", 12, "bold")).grid(row=1, column=0, sticky="w", pady=5)
        ...
        if "recurrence_end_date" in self.event_data and self.event_data["recurrence_end_date"]:
            recurrence_end_date = datetime.fromisoformat(self.event_data["recurrence_end_date"])
            self.recurrence_end_date_var = tk.StringVar(value=recurrence_end_date.strftime("%Y-%m-%d"))
        else:
            self.recurrence_end_date_var = tk.StringVar(value=default_end_date.strftime("%Y-%m-%d"))
        
        ttk.Label(end_recurrence_frame, text=self._("Date:"), style="TLabel").pack(side=tk.LEFT, padx=(0, 5))
    ...
    def show_date_picker(self, entry_widget):
        ...
        # Apply theme
        date_window.configure(bg=self.app.theme_manager.themes[self.app.theme_manager.current_theme]["bg"])
        ...
        # Header
        header_frame = ttk.Frame(date_frame, style="Header.TFrame")
                ...
                if day != 0:
                    ...
                    if is_today:
                        ...
                        day_canvas.create_text(
                            23, 23, 
                            text=str(day), 
                            fill=self.app.theme_manager.themes[self.app.theme_manager.current_theme]["fg"], 
                    ...
                    def on_enter(e, canvas=day_canvas, is_today=is_today):
                        if not is_today:
                            canvas.create_oval(
                                8, 8, 38, 38, 
                                fill=self.app.theme_manager.themes[self.app.theme_manager.current_theme]["button"], 
                                outline="",
                                tags="hover"
                            )
                    ...
                    day_canvas.bind(
                        "<Button-1>", 
                        lambda e, y=year, m=month, d=day: self.select_date(y, m, d, date_window, entry_widget)
                    )
        ...
        try:
            ...
            if end_time <= start_time:
                messagebox.showerror(
                    self._("Error"), 
                    self._("End time must be after start time")
                )
                return
            ...
            if self.is_recurring_var.get():
                recurrence_end_date = datetime.strptime(
                    self.recurrence_end_date_var.get(), 
                    "%Y-%m-%d"
            ...
            event_data = {
                ...
                "recurrence_type": self.recurrence_type_var.get() if self.is_recurring_var.get() else None,
            ...
            if self.event_data and "id" in self.event_data:
                ...
                messagebox.showinfo(
                    self._("Success"), 
                    self._("Event updated successfully")
                )
            else:
                # Create new event
                ...
                messagebox.showinfo(
                    self._("Success"), 
                    self._("Event created successfully")
                )
    ...
    def show_event_history(self):
        ...
        if not history:
            messagebox.showinfo(
                self._("Event History"),
                self._("No history found for this event")
            )
            return
        ...
        # Apply theme
        history_window.configure(bg=self.app.theme_manager.themes[self.app.theme_manager.current_theme]["bg"])
        
        # Create a header
        header_frame = ttk.Frame(history_window, style="Header.TFrame")
        ...
        scrollbar = ttk.Scrollbar(history_frame, orient=tk.VERTICAL, command=canvas.yview)
        ...
        for entry in history:
            ...
            action_color = self.app.theme_manager.themes[self.app.theme_manager.current_theme]["fg"]
            if entry["action"] == "create":
                action_color = self.app.theme_manager.themes[self.app.theme_manager.current_theme]["success"]
            elif entry["action"] == "update":
                action_color = self.app.theme_manager.themes[self.app.theme_manager.current_theme]["warning"]
            elif entry["action"] == "delete":
                action_color = self.app.theme_manager.themes[self.app.theme_manager.current_theme]["danger"]
